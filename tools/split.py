...
def split_coco(src, val, out, verbose=False):
    ...
    ids_val = {
        ...
        if im['file_name'] in VAL}

    # extract images and anns in train set, save new train dataset
    images_train = [
        im for im in images
        if im['id'] not in ids_val]
    anns_train = [
        ann for ann in annotations
        if ann['image_id'] not in ids_val]

    if verbose:
        print(
            'Train set:',
            len(images_train), 'images,',
            len(anns_train), 'annotations.')

    dst_train = out / 'train.json'
    save_coco(
        dst_train,
        images_train, anns_train,
    ...
    anns_val = [
        ann for ann in annotations
        if ann['image_id'] in ids_val]

    if verbose:
        print(
            'Validation set:',
            len(images_val), 'images,',
            len(anns_val), 'annotations.')
