...
if __name__ == "__main__":
    ...
    rois_dir = args.out_dir / args.folder
    rois_dir.mkdir()
    ...
    obj_id = 1
    fnames = get_image_files(args.data_dir)

    # extract ROIs
    for image_path in tqdm(fnames):
        # get path to txt predictions file for this image
        txt_name = image_path.stem + ".txt"
        txt_path = args.labels_dir / txt_name
        ...
        for label, box_xywh, score in output:
            ...
            im_crop = im.crop(
                (left - args.pad, upper - args.pad, right + args.pad, lower + args.pad)
            )
            name_obj = f"{args.prefix}{obj_id}_{label}.jpg"
    ...
    with open(args.out_dir / args.file, 'w') as fout:
