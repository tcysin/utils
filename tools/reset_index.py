...
def reset_index(src, dst, verbose=False):
    ...
    if verbose:
        print("Original:", len(images), "images,", len(annotations), "annotations.")
    ...
    if verbose:
        print(
            "After cleaning:", len(images), "images,", len(annotations), "annotations."
        )
    ...
    old2new_images = {
        image["id"]: new_id for (new_id, image) in enumerate(images, start=1)
    }
    ...
    old2new_categories = {
        category["id"]: new_id for (new_id, category) in enumerate(categories, start=1)
    }
    ...
    old2new_anns = {
        ann["id"]: new_id for (new_id, ann) in enumerate(annotations, start=1)
    }
